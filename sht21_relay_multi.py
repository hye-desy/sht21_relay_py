...
if __name__ == "__main__":
    ...
    file_dat=open("sht21_"+time.ctime().replace("  "," ").replace(" ","_").replace(":","-")+".dat","w")
    temp=0
    hr=0
    dewpoint=0
    dewpoint_list=[]
    ...
    try:
	    ...
	    for i in range(0,len(gpio)):
	            ...
	            temp=sht21.read_temperature()
                    ...
                    print "%s GPIO:%s, T: %s, HR: %s, DewPoint: %s" % (timestamp,i, temp,hr,dewpoint)
	            file_dat.write(timestamp+str(i)+" "+str(temp)+" "+str(hr)+" "+str(dewpoint)+"\n")
            ...
            if all(i <DewPoint_alarm for i in dewpoint_list) and relay_get_port_status(2) is False:
	        relay_on(2)
                print "Alarm reset."
